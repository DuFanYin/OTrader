...
class ExecutionEngine : public utilities::BaseEngine {
  public:
    ...
    explicit ExecutionEngine(runtime_common::MainEngineBase* main)
        : BaseEngine(main, "Execution") {}
    ...
    /** Process order event; remove from active if CANCELLED/REJECTED/ALLTRADED. */
    void process_order_event(const std::string& strategy_name, const utilities::OrderData& order);
