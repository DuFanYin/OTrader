...
class OptionStrategyEngine : public utilities::BaseEngine {
  public:
    OptionStrategyEngine(runtime_common::MainEngineBase* main, RuntimeAPI api);
    ~OptionStrategyEngine() override;

    /** Load strategy defaults from Otrader/strategy_config.json. */
    void load_strategy_config();

    void process_order_event(utilities::OrderData& order);
    void process_trade_event(const utilities::TradeData& trade);
    ...
    /** Cached strategy names (no per-call allocation); invalidated on add/remove. */
    const std::vector<std::string>& get_strategy_names_ref() const;
    /** Pairs (strategy_name, error_msg) for all strategies with error() == true (e.g. for Live
     * monitoring / gRPC). */
  ...
  private:
    ...
    std::unordered_map<std::string, std::unique_ptr<strategy_cpp::OptionStrategyTemplate>>
        strategies_;
    mutable std::vector<std::string> strategy_names_cache_;
    mutable bool strategy_names_dirty_ = true;
