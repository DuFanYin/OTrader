...
OptionStrategyEngine::OptionStrategyEngine(runtime_common::MainEngineBase* main, RuntimeAPI api)
...
void OptionStrategyEngine::load_strategy_config() {
    ...
    constexpr const char* kPath = utilities::config::kStrategyConfigPath;
...
void OptionStrategyEngine::process_order_event(utilities::OrderData& order) {
...
void OptionStrategyEngine::process_trade_event(const utilities::TradeData& trade) {
    std::string strategy_name = api_.execution.get_strategy_name_for_order
...
void OptionStrategyEngine::write_log(const std::string& msg, int level) const {
    // Use BaseEngine unified logging API; gateway fixed to \"Strategy\".
    utilities::BaseEngine::write_log(msg, level, "Strategy");
}
...
void OptionStrategyEngine::process_timer_event() {
...
auto OptionStrategyEngine::get_strategy_names_ref() const -> const std::vector<std::string>& {
    if (strategy_names_dirty_) {
        strategy_names_cache_.clear();
        strategy_names_cache_.reserve(strategies_.size());
        for (const auto& [name, _] : strategies_) {
            strategy_names_cache_.push_back(name);
        }
        strategy_names_dirty_ = false;
    }
    return strategy_names_cache_;
}

auto OptionStrategyEngine::get_strategy_errors() const
    -> std::vector<std::pair<std::string, std::string>> {
    std::vector<std::pair<std::string, std::string>> out;
    for (const auto& [name, ptr] : strategies_) {
        if (ptr && ptr->error()) {
            out.emplace_back(name, ptr->error_msg());
        }
    }
    return out;
}
