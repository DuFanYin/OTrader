...
class HedgeEngine : public utilities::BaseEngine {
  public:
    HedgeEngine() = default;
    explicit HedgeEngine(runtime_common::MainEngineBase* main) : BaseEngine(main, "Hedge") {}
    void set_runtime_api(core::RuntimeAPI api) { api_ = std::move(api); }
    ...
    /** Run hedging; append OrderRequest/CancelRequest to caller vectors. */
  ...
  private:
    void run_strategy_hedging_with_params(const std::string& strategy_name,
