...
std::string portfolio_name_for_underlying(const std::string& symbol_prefix) {
    auto it = utilities::config::kUnderlyingToPortfolio.find(symbol_prefix);
    return (it != utilities::config::kUnderlyingToPortfolio.end()) ? it->second : symbol_prefix;
}

} // namespace

void PortfolioStructure::ensure_portfolios_created() {
    for (const std::string& name : utilities::config::kDefaultPortfolioNamesToCreate) {
