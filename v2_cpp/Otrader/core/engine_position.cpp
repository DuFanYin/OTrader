...
void PositionEngine::process_order_event(const std::string& strategy_name,
...
void PositionEngine::process_trade_event(const std::string& strategy_name,
...
void PositionEngine::apply_position_change(utilities::OptionPositionData* pos,
                                           const utilities::TradeData& trade) {
    // OptionPositionData is-a BasePosition. Use the same avg_cost/realized_pnl logic so that
    // single-leg option positions and combo heads behave consistently.
    apply_position_change(static_cast<utilities::BasePosition*>(pos), trade);
}
...
auto PositionEngine::accumulate_option_position(utilities::OptionPositionData& opt,
    ...
    opt.cost_value = round_digits(total_cost, 2);
    opt.realized_pnl = round_digits(realized, 2);
