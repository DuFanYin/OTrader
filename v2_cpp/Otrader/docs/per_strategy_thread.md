...
- **EventEngine** 持有一条 `strategy_thread_`，跑 `run_strategy()`。
- **strategy_ring_**（SPSC）由 main worker 推入事件（Timer / Order / Trade），strategy_thread_ 顺序 pop → `process_strategy()`。
- **OptionStrategyEngine::on_timer()** 在同一线程内遍历 `strategies_`，依次调用每个策略的 `s->on_timer()`。
- Order/Trade 事件按 orderid 查 strategy_name，再调对应策略的 `process_order` / `process_trade`，仍在同一线程。

结论：多策略共享一条线程，一个策略卡住会拖慢其他策略。
