...
| 脚本 | 默认 BUILD 目录 | 职责 |
|------|------------------|------|
| **build.sh** | `Otrader/build`（`BUILD_DIR`） | 创建目录、**唯一**执行 cmake 配置；**只编**聚合 target `otrader_main`（= `entry_backtest` + `entry_system` 两个可执行文件），不编任何 test。 |
...
聚合 target `otrader_main`，即两个可执行文件：

- `entry_backtest`
- `entry_system`（统一实盘入口，`--mode=gateway|market|live|all`；由 gateway/market/live 各组件链入）

（以及它们依赖的库；**不**包含 `test_*`、`otrader_unit_test`。strategy-only 模式 `./build.sh s` 只编 `strategies` + `entry_backtest`。）
...
| `BUILD_GATEWAY` | OFF | entry_system 的 gateway 组件 | `infra/gateway` → `gateway_core`，需 cppzmq |
| `BUILD_MARKETDATA` | OFF | entry_system 的 market data 组件 | 可能拉入 `runtime/live`，链 engines_cpp + cppzmq |
| `BUILD_LIVE_GRPC` | OFF | entry_system 的 live 运行时 + gRPC | 拉入 `runtime/live`，链 engines_cpp |
| `BUILD_TESTS` | OFF | 启用 CTest + tests 子目录 | `add_subdirectory(tests)`，**不**自动开 BACKTEST/LIVE_GRPC |

> `entry_system` 由 `BUILD_GATEWAY`、`BUILD_MARKETDATA`、`BUILD_LIVE_GRPC` 中任一开启即构建，三个组件分别链入 gateway_core / cppzmq / engines_cpp。三者不再产生独立可执行文件。
...
当前 **build.sh** 的配置为：`BUILD_BACKTEST`、`BUILD_TESTS`、`BUILD_LIVE_GRPC`、`BUILD_GATEWAY`、`BUILD_MARKETDATA` 全 ON，因此该 build 树中**存在** `entry_backtest`、`entry_system` 及全部 test target；由脚本决定**谁去编**谁。
...
| 位置 | Target | 类型 | 说明 |
|------|--------|------|------|
| `infra/gateway/` | `gateway_core` | 库 | IB Gateway + ZMQ，依赖 TWS；链入 `entry_system` 的 gateway 组件 |

### 4.3 BUILD_MARKETDATA=ON

| 位置 | Target | 类型 | 说明 |
|------|--------|------|------|
| `infra/marketdata/` | market data 组件 | 库 | 链入 `entry_system`（可能拉入 engines_cpp + cppzmq） |

### 4.4 BUILD_LIVE_GRPC=ON

| 位置 | Target | 类型 | 说明 |
|------|--------|------|------|
| `runtime/live/` | `engines_cpp` | 库 | 实盘运行时 + gRPC，链入 `entry_system` 的 live 组件 |

### 4.5 统一可执行文件 entry_system

| 位置 | Target | 类型 | 条件 |
|------|--------|------|------|
| 根目录 | `entry_system` | 可执行文件 | `BUILD_GATEWAY` / `BUILD_MARKETDATA` / `BUILD_LIVE_GRPC` 任一 ON；统一实盘入口 `--mode=gateway\|market\|live\|all` |
...
strategies ← option_strategy_engine_shared
    ↑
backtest_engines (BUILD_BACKTEST)   engines_cpp (BUILD_LIVE_GRPC) / gateway_core (BUILD_GATEWAY)
    ↑                                    ↑
entry_backtest, test_backtest_*      entry_system, test_live_*
```
...
| `entry_system` | BUILD_GATEWAY / BUILD_MARKETDATA / BUILD_LIVE_GRPC 任一 ON | build.sh |
