...
├── entry_system.cpp                         # 统一实盘入口（--mode=gateway|market|live|all）
│
├── runtime/                                 # 运行时：回测与实盘差异集中在此
│   ├── main_engine_base.{cpp,hpp}           # MainEngine 共享基类
│   ├── backtest/
...
│       ├── gateway_client.{cpp,hpp}         # 连接网关进程的 ZMQ 客户端
│       └── market_data_client.{cpp,hpp}     # 连接行情进程的 ZMQ 客户端
│
├── infra/                                   # 基础设施：数据、持久化、网关
│   ├── marketdata/
│   │   ├── engine_data_historical.{cpp,hpp} # 回测数据引擎（parquet → 快照）
│   │   ├── engine_data_tradier.{cpp,hpp}    # 实盘行情/组合引擎
│   │   └── zmq_marketdata_schema.{cpp,hpp}  # ZMQ 行情消息 schema
│   ├── db/
│   │   └── engine_db_pg.{cpp,hpp}           # PostgreSQL 合约/订单/成交
│   └── gateway/
│       ├── engine_gateway_ib.{cpp,hpp}      # IB TWS 网关
│       ├── engine_gateway_backtest.{cpp,hpp}# 回测撮合网关
│       └── zmq_gateway_schema.{cpp,hpp}     # ZMQ 网关消息 schema
│
├── core/                                    # 领域核心：策略引擎与辅助引擎
...
│   ├── portfolio_structure.{cpp,hpp}        # 组合结构辅助
│   └── runtime_api.hpp                      # RuntimeAPI（Execution/Portfolio/System 分组）
│
├── strategy/                                # 策略实现与注册
│   ├── factory/                             # 具体策略实现
...
│
├── proto/                                   # Protobuf/gRPC 与 ZMQ 消息定义（.proto）
├── tests/                                   # 回测与实盘测试（GTest）
├── thirdparty/                              # 第三方依赖（IB JTS、IntelRDFP、lets_be_rational）
└── utilities/                               # 通用数据模型与抽象
...
| **实盘（gRPC）** | 行情来自 MarketDataEngine（如 Tradier）；通过 IB TWS 下单并记录成交，并通过 gRPC 对外暴露控制接口 | `entry_system --mode=live` | 在 `0.0.0.0:50051` 暴露 EngineService；合约与订单/成交持久化到 PostgreSQL；经 ZMQ 连接网关/行情进程 |
| **网关** | 独立的 IB TWS 网关进程；接收下单/撤单请求，经 ZMQ 发布订单/成交回报 | `entry_system --mode=gateway` | ZMQ REP+PUB；不含策略逻辑或数据库 |
| **行情** | 独立的行情进程；经 ZMQ 发布组合快照 | `entry_system --mode=market` | ZMQ REP+PUB；需要 DATABASE_URL 与行情源（如 Tradier） |

> 实盘系统由单一 `entry_system` 二进制启动为三个进程：**gateway**、**market**、**live**（`--mode=all` 会先以子进程启动 gateway + market，再在前台运行 live），彼此通过 ZeroMQ IPC 通信。统一的 `entry_system` 取代了此前分离的 `entry_gateway` / `entry_market_data` / `entry_live_grpc` 可执行文件。

### 1.2 架构分层

| 层次 | 职责 | 主要组件 |
|------|------|----------|
| **Domain Core** | 纯逻辑：接收 Event、更新状态、输出 Intent；不直接下单、不读库、不访问网关 | OptionStrategyEngine、PositionEngine、HedgeEngine、LogEngine、ExecutionEngine、Strategy 实现，以及组合构造工具（`utilities::combo`） |
...
**所有权**：MainEngine 持有所有引擎实例（EventEngine、LogEngine、OptionStrategyEngine、PositionEngine、HedgeEngine、ExecutionEngine）。基础设施的所有权按运行时不同：回测 MainEngine 持有 BacktestDataEngine 与回测撮合网关；实盘 MainEngine 持有 DatabaseEngine 以及 MarketDataClient、GatewayClient（连接独立行情/网关进程的 ZMQ 客户端）——真正的 MarketDataEngine 与 IbGateway 运行在那两个独立进程中。组合构造为无状态工具（`utilities::combo` 自由函数），非被持有的引擎。EventEngine 持有 MainEngine 的非拥有指针，通过 MainEngine 的 accessor 访问各引擎，自身不直接持有引擎实例。

**Event 流向**：外部生产者调用 MainEngine 的 `put_event`，MainEngine 转发给 EventEngine。回测中生产者为 BacktestEngine 与回测撮合网关；实盘中为 MarketDataClient 与 GatewayClient，它们经 ZMQ 从独立的行情/网关进程接收 Snapshot 与 Order/Trade。EventEngine 按类型分发：Snapshot → 组合 `apply_frame`；Timer → OptionStrategyEngine（驱动策略、PositionEngine、HedgeEngine、ExecutionEngine）；Order/Trade → ExecutionEngine 与 PositionEngine 更新状态，再经 OptionStrategyEngine 回调策略的 `on_order`/`on_trade`。

**Intent 流向**：策略与 HedgeEngine 通过 RuntimeAPI（send_order、cancel_order、write_log）产出 Intent。RuntimeAPI 绑定到 MainEngine：订单/撤单 Intent 进入 EventEngine 的 `put_intent`（实盘）或 BacktestEngine 的撮合路径（回测）；日志 Intent 进入 LogEngine。OptionStrategyEngine 在构造时接收 RuntimeAPI；HedgeEngine 在需要时通过 SystemAPI（`get_hedge_engine`）获取，而组合构造直接调用 `utilities::combo` 自由函数。

**Core 隔离**：OptionStrategyEngine、PositionEngine、HedgeEngine、LogEngine、ExecutionEngine 不持有 MainEngine 或 EventEngine，所需能力通过 RuntimeAPI 或 caller 传入的回调（如 `get_portfolio`、`send_impl`）获得；组合构造工具的 `get_contract` 回调同样由 caller 传入。
...
| **组合构造**（`utilities::combo`） | 按 ComboType 与期权数据生成标准化 Leg 与组合签名（`build_combo`、`create_leg`、`generate_combo_signature`） | 无状态自由函数，非引擎；get_contract 由 caller 传入 |
...
| **Live 实盘** | EventEngine 使用队列与定时器线程；MainEngine 持有 DatabaseEngine、MarketDataClient、GatewayClient（连接独立行情/网关进程的 ZMQ REQ+SUB）；构造时 load_contracts 建立组合结构；下单/撤单转 GatewayClient；dispatch_order / dispatch_trade 内 save_order_data / save_trade_data | MarketDataClient/GatewayClient 经 ZMQ 订阅；Snapshot 与 Order/Trade 作为事件从这些流到达。合约由 load_contracts 回调建立，无 Contract 事件入队 |
