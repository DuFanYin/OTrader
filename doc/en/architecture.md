...
├── entry_system.cpp                         #   Unified live entry (--mode=gateway|market|live|all)
│
├── runtime/                                 #   Runtime: backtest vs live differences
│   ├── main_engine_base.{cpp,hpp}           #   Shared MainEngine base
│   ├── backtest/
...
│       ├── gateway_client.{cpp,hpp}         #   ZMQ client to the gateway process
│       └── market_data_client.{cpp,hpp}     #   ZMQ client to the market-data process
│
...
│   │   └── zmq_marketdata_schema.{cpp,hpp}  #   ZMQ market-data message schema
│   ├── db/
│   │   └── engine_db_pg.{cpp,hpp}           #   PostgreSQL contract/order/trade
│   └── gateway/
│       ├── engine_gateway_ib.{cpp,hpp}      #   IB TWS gateway
│       ├── engine_gateway_backtest.{cpp,hpp}#   Backtest matching gateway
│       └── zmq_gateway_schema.{cpp,hpp}     #   ZMQ gateway message schema
│
...
│   ├── portfolio_structure.{cpp,hpp}        #   Portfolio structure helpers
│   └── runtime_api.hpp                      #   RuntimeAPI (Execution/Portfolio/System groups)
│
...
│
├── proto/                                   #   Protobuf/gRPC + ZMQ message definitions (.proto)
├── tests/                                   #   Backtest and live tests (GTest)
├── thirdparty/                              #   Third-party deps (IB JTS, IntelRDFP, lets_be_rational)
└── utilities/                               #   Shared data models and abstractions
    ...
    ├── combo_builder.{cpp,hpp}              #   Combo leg builder
...
| **Live (gRPC)** | Market data from MarketDataEngine (e.g. Tradier); submit orders and record fills via IB TWS, and expose external control via gRPC | `entry_system --mode=live` | EngineService at `0.0.0.0:50051`; contracts and orders/trades in PostgreSQL; connects to the gateway/market processes over ZMQ |
| **Gateway** | Standalone IB TWS gateway process; receives order/cancel requests and publishes order/trade reports over ZMQ | `entry_system --mode=gateway` | ZMQ REP+PUB; no strategy logic or database |
| **Market Data** | Standalone market-data process; publishes portfolio snapshots over ZMQ | `entry_system --mode=market` | ZMQ REP+PUB; requires DATABASE_URL and a market-data source (e.g. Tradier) |

> The live system is split into three processes launched from the single `entry_system` binary: **gateway**, **market**, and **live** (`--mode=all` starts gateway + market as subprocesses, then runs live in the foreground). They communicate over ZeroMQ IPC. The unified `entry_system` replaces the earlier separate `entry_gateway` / `entry_market_data` / `entry_live_grpc` executables.

### 1.2 Architecture Layers

| Layer | Responsibility | Main Components |
|-------|----------------|-----------------|
| **Domain Core** | Pure logic: receive Events, update state, emit Intents; no direct order submission, DB access, or gateway access | OptionStrategyEngine, PositionEngine, HedgeEngine, LogEngine, ExecutionEngine, Strategy implementations, and the combo builder utility (`utilities::combo`) |
...
**Ownership**: MainEngine owns all engine instances (EventEngine, LogEngine, OptionStrategyEngine, PositionEngine, HedgeEngine, ExecutionEngine). Combo building is a stateless utility (`utilities::combo` free functions), not an owned engine. Infrastructure ownership differs by runtime: the backtest MainEngine owns BacktestDataEngine and a matching gateway; the live MainEngine owns DatabaseEngine plus MarketDataClient and GatewayClient (ZMQ clients to the separate market-data and gateway processes) — the actual MarketDataEngine and IbGateway run in those standalone processes. EventEngine holds a non-owning pointer to MainEngine and accesses engines via MainEngine accessors; it does not hold engine instances directly.

**Event flow**: External producers call `put_event` on MainEngine, which forwards to EventEngine. In backtest these are BacktestEngine and the backtest matching gateway; in live they are MarketDataClient and GatewayClient, which receive Snapshot and Order/Trade over ZMQ from the separate market-data and gateway processes. EventEngine dispatches by type: Snapshot → portfolio `apply_frame`; Timer → OptionStrategyEngine (which drives strategies, PositionEngine, HedgeEngine, and ExecutionEngine); Order/Trade → ExecutionEngine and PositionEngine for state update, then OptionStrategyEngine for strategy `on_order`/`on_trade` callbacks.

**Intent flow**: Strategies and HedgeEngine produce Intents via RuntimeAPI (send_order, cancel_order, write_log). RuntimeAPI is wired to MainEngine: order/cancel intents go to EventEngine's `put_intent` (live) or BacktestEngine's matching path (backtest); log intents go to LogEngine. OptionStrategyEngine receives RuntimeAPI at construction; the HedgeEngine is obtained via SystemAPI (`get_hedge_engine`) when needed, while combo building calls the `utilities::combo` free functions directly.

**Core isolation**: OptionStrategyEngine, PositionEngine, HedgeEngine, LogEngine, and ExecutionEngine do not hold MainEngine or EventEngine. They receive capabilities via RuntimeAPI or caller-passed callbacks (e.g. `get_portfolio`, `send_impl`); the combo builder utility takes its `get_contract` callback from the caller.
...
| **SystemAPI** | Logging, helper engines (hedge), strategy event push |
...
| **Combo builder** (`utilities::combo`) | Generate standardized Legs and combo signatures from ComboType and option data (`build_combo`, `create_leg`, `generate_combo_signature`) | Stateless free functions, not an engine; get_contract passed by caller |
...
| **Live** | EventEngine uses queue and timer thread; MainEngine holds DatabaseEngine, MarketDataClient, and GatewayClient (ZMQ REQ+SUB to the separate market-data and gateway processes); load_contracts at construction sets up portfolio structure; order/cancel go to GatewayClient; save_order_data / save_trade_data in dispatch_order / dispatch_trade | MarketDataClient/GatewayClient subscribe over ZMQ; Snapshot and Order/Trade arrive as events from those streams. Contracts built by load_contracts callback without enqueuing a Contract event |
