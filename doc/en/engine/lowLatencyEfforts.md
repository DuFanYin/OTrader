...
These designs are in place today for both live and backtest where applicable. One planned evolution — **not yet shipped** — is moving from the single shared strategy thread to one thread per strategy. For the concrete runtime thread/queue layout see [liveThreadModel](./liveThreadModel.md); for the zero-copy data path see [zeroCopyDataPath](./zeroCopyDataPath.md).
