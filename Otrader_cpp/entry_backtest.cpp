...
int main(int argc, char* argv[]) {
    ...
    if (std::string(argv[1]) == "--files") {
        ...
        while (arg_idx < argc) {
            ...
            if (arg == "--fee-rate" || arg == "--slippage-bps" ||
                arg == "--risk-free-rate" || arg == "--iv-price-mode" || arg == "--log" ||
    ...
    try {
        ...
        if (parquet_files.size() == 1) {
            ...
            std::vector<backtest::BacktestEngine> engines(num_engines);
            for (auto& eng : engines) {
                eng.configure_execution(fee_rate, slippage_bps);
                eng.main_engine()->set_log_level(log_level);
            }
            ...
            auto worker = [&](int engine_id) {
                while (true) {
                    ...
                    auto& file_engine = engines[engine_id];
                    std::vector<Metric> file_metrics;
                    file_metrics.reserve(400);
                    file_engine.reset();
                    file_engine.register_timestep_callback([&](int timestep, backtest::Timestamp ts) {
            ...
            for (size_t i = 0; i < metrics.size(); ++i) {
                ...
                if (date_str != prev_date) {
                    if (!prev_date.empty() && i > 0) {
                        // Shift next day's curve by previous day's close (relative to its own start).
        ...
        if (n_metrics > 0) {
            ...
            for (size_t i = 0; i < n_metrics; ++i) {
                ...
                if (date_str != prev_date) {
                    if (!prev_date.empty() && i > 0) {
                        // metrics[i-1].pnl is cumulative from start; day_start_pnl was start of previous day
                        // So previous day's total PnL = metrics[i-1].pnl - day_start_pnl
                        cumulative_offset += metrics[i - 1].pnl - day_start_pnl;
                ...
                // metrics[i].pnl is cumulative from start; subtract day_start_pnl to get relative to day start,
                // then add cumulative_offset to stack days
