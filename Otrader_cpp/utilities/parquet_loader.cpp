...
Timestamp ArrowTsToChrono(int64_t value, TimeUnit::type unit) {
...
std::string TsToIso(Timestamp ts) {
...
inline const Array* ColumnChunk0(const Table* table, int col) {
...
public:
    bool load(std::string const& path, std::string const& time_column) override {
        meta_.path = path;
        ...
        // 2) 创建 Parquet reader（Arrow 新 API：OpenFile 两参数，返回 Result<unique_ptr<FileReader>>）
        ...
        if (time_col_index_ < 0) return false;

        if (meta_.row_count > 0) {
            const Array* ts_arr = ColumnChunk0(table_.get(), time_col_index_);
            if (ts_arr && ts_arr->type_id() == Type::TIMESTAMP) {
                auto* ts = static_cast<const TimestampArray*>(ts_arr);
    ...
    DataMeta get_meta() const override { return meta_; }
    ...
    void collect_symbols(std::unordered_set<std::string>& out) const override {
        ...
        if (!arr || arr->type_id() != Type::STRING || arr->null_count() == arr->length()) return;
    ...
    void iter_timesteps(std::function<bool(TimestepFrameColumnar const&)> const& fn) const override {
        if (!table_ || time_col_index_ < 0) return;
        const Array* ts_arr = ColumnChunk0(table_.get(), time_col_index_);
        if (!ts_arr || ts_arr->type_id() != Type::TIMESTAMP) return;
        ...
        frame.arr_underlying_bid_px = col_ubid >= 0 ? ColumnChunk0(table_.get(), col_ubid) : nullptr;
        frame.arr_underlying_ask_px = col_uask >= 0 ? ColumnChunk0(table_.get(), col_uask) : nullptr;
        frame.arr_underlying_bid_sz = col_ubid_sz >= 0 ? ColumnChunk0(table_.get(), col_ubid_sz) : nullptr;
        frame.arr_underlying_ask_sz = col_uask_sz >= 0 ? ColumnChunk0(table_.get(), col_uask_sz) : nullptr;
        ...
        for (auto const& [t, _] : groups) sorted_ts.push_back(t);
