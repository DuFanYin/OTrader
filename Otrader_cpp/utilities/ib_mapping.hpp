...
inline std::string direction_vt2ib(utilities::Direction d) {
    switch (d) {
        case utilities::Direction::LONG: return "BUY";
        case utilities::Direction::SHORT: return "SELL";
        default: return "BUY";
    }
}
...
inline utilities::Direction direction_ib2vt(const std::string& action) {
    if (action == "BUY" || action == "BOT") return utilities::Direction::LONG;
    if (action == "SELL" || action == "SLD") return utilities::Direction::SHORT;
...
inline std::string ordertype_vt2ib(utilities::OrderType t) {
    switch (t) {
        case utilities::OrderType::LIMIT: return "LMT";
        case utilities::OrderType::MARKET: return "MKT";
        default: return "LMT";
    }
}
...
inline utilities::OrderType ordertype_ib2vt(const std::string& t) {
    if (t == "MKT") return utilities::OrderType::MARKET;
...
inline utilities::Exchange exchange_ib2vt(const std::string& exch) {
    static const std::unordered_map<std::string, utilities::Exchange> m = {
        {"SMART", utilities::Exchange::SMART},
        {"NYSE", utilities::Exchange::NYSE},
        {"NASDAQ", utilities::Exchange::NASDAQ},
        {"AMEX", utilities::Exchange::AMEX},
        {"CBOE", utilities::Exchange::CBOE},
        {"IBKRATS", utilities::Exchange::IBKRATS},
    };
    auto it = m.find(exch);
...
inline std::string exchange_vt2ib(utilities::Exchange e) {
    return utilities::to_string(e);
}
...
inline utilities::Product product_ib2vt(const std::string& sec_type) {
    static const std::unordered_map<std::string, utilities::Product> m = {
        {"STK", utilities::Product::EQUITY},
        {"OPT", utilities::Product::OPTION},
        {"FOP", utilities::Product::OPTION},
        {"IND", utilities::Product::INDEX},
...
inline utilities::OptionType option_ib2vt(const std::string& right) {
    if (right == "C" || right == "CALL") return utilities::OptionType::CALL;
    if (right == "P" || right == "PUT") return utilities::OptionType::PUT;
