...
public:
    ...
    void load_parquet(std::string const& rel_path,
                      std::string const& time_column = "ts_recv",
    ...
    void iter_timesteps(std::function<bool(Timestamp, TimestepFrameColumnar const&)> const& fn) const;
    ...
    /** After load, snapshots are precomputed (one per frame). Run uses these; no per-frame IV/Greeks. */
    [[nodiscard]] size_t get_precomputed_snapshot_count() const { return snapshots_.size(); }
    [[nodiscard]] utilities::PortfolioSnapshot const& get_precomputed_snapshot(size_t i) const { return snapshots_.at(i); }
...
private:
    ...
    /** One-time: parquet OCC symbol -> OptionData* (so build_snapshot does not parse or options.find per row). */
    void build_occ_to_option(std::unordered_set<std::string> const& occ_symbols);
    /** Build one snapshot from frame; if prev is non-null, copy its values first so options not in this frame keep last state. */
    utilities::PortfolioSnapshot build_snapshot_from_frame(TimestepFrameColumnar const& frame,
    ...
    /** One-time at load: parquet raw (OCC) symbol -> OptionData*; build phase only does find, no parse. */
