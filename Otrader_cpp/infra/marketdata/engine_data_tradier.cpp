...
void MarketDataEngine::process_contract(const utilities::Event& event) {
    if (const auto* p = std::get_if<utilities::ContractData>(&event.data)) {
        ...
        std::string underlying_name =
            (pos != std::string::npos) ? p->symbol.substr(0, pos) : p->symbol;
        ...
        } else if (p->product == utilities::Product::EQUITY ||
                   p->product == utilities::Product::INDEX) {
...
void MarketDataEngine::stop_market_data_update() { started_ = false; }
