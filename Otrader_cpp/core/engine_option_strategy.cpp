...
std::string extract_class_name(const std::string& strategy_name) {
    std::size_t pos = strategy_name.find('_');
    if (pos == std::string::npos) return strategy_name;
    return strategy_name.substr(0, pos);
}

std::string extract_portfolio_name(const std::string& strategy_name) {
    std::size_t pos = strategy_name.find('_');
    if (pos == std::string::npos || pos + 1 >= strategy_name.size()) return strategy_name;
...
void OptionStrategyEngine::process_trade(const utilities::TradeData& trade) {
    ...
    if (it != orderid_strategy_name_.end()) {
        ...
        if (s) s->on_trade(trade);
    }
}

strategy_cpp::OptionStrategyTemplate* OptionStrategyEngine::get_strategy(const std::string& strategy_name) {
    auto it = strategies_.find(strategy_name);
    return (it != strategies_.end()) ? it->second.get() : nullptr;
}

strategy_cpp::OptionStrategyTemplate* OptionStrategyEngine::get_strategy() {
    if (strategies_.size() != 1) return nullptr;
    return strategies_.begin()->second.get();
}

utilities::StrategyHolding* OptionStrategyEngine::get_strategy_holding(const std::string& strategy_name) const {
    if (!api_.get_holding) return nullptr;
    return api_.get_holding(strategy_name);
}

utilities::StrategyHolding* OptionStrategyEngine::get_strategy_holding() const {
    if (strategies_.size() != 1) return nullptr;
    return get_strategy_holding(strategies_.begin()->first);
}

utilities::PortfolioData* OptionStrategyEngine::get_portfolio(const std::string& portfolio_name) {
    return api_.get_portfolio ? api_.get_portfolio(portfolio_name) : nullptr;
}

utilities::StrategyHolding* OptionStrategyEngine::get_holding(const std::string& strategy_name) {
    return api_.get_holding ? api_.get_holding(strategy_name) : nullptr;
}

const utilities::ContractData* OptionStrategyEngine::get_contract(const std::string& symbol) const {
    return api_.get_contract ? api_.get_contract(symbol) : nullptr;
}

void OptionStrategyEngine::write_log(const std::string& msg, int level) {
    if (api_.write_log) {
...
void OptionStrategyEngine::write_log(const utilities::LogData& log) {
    if (api_.write_log) api_.write_log(log);
}

std::string OptionStrategyEngine::send_order(const std::string& strategy_name, const utilities::OrderRequest& req) {
    if (!api_.send_order) return {};
...
std::vector<std::string> OptionStrategyEngine::send_order(const std::string& strategy_name, const std::string& symbol,
...
std::vector<std::string> OptionStrategyEngine::send_combo_order(const std::string& strategy_name,
                                                                ...
                                                                utilities::OrderType order_type) {
    if (!api_.send_combo_order || legs.empty()) return {};
    std::string orderid = api_.send_combo_order(strategy_name, combo_type, combo_sig, direction, price, volume, legs, order_type);
    if (!orderid.empty()) {
        strategy_active_orders_[strategy_name].insert(orderid);
        orderid_strategy_name_[orderid] = strategy_name;
        all_active_order_ids_.insert(orderid);
...
void OptionStrategyEngine::init_strategy(const std::string& strategy_name) {
    auto* s = get_strategy(strategy_name);
    if (!s) throw std::runtime_error("Strategy not found: " + strategy_name);
    s->on_init();
...
bool OptionStrategyEngine::remove_strategy(const std::string& strategy_name) {
    auto it = strategies_.find(strategy_name);
    if (it == strategies_.end()) return false;
    if (it->second) it->second->on_stop();
    ...
    if (api_.remove_strategy_holding) api_.remove_strategy_holding(strategy_name);
    if (api_.put_strategy_event) {
        utilities::StrategyUpdateData u;
        u.strategy_name = strategy_name;
        u.class_name = extract_class_name(strategy_name);
...
void OptionStrategyEngine::add_strategy(const std::string& class_name,
    ...
    void* raw = strategy_cpp::StrategyRegistry::create(class_name, this, strategy_name, portfolio_name, setting);
    if (!raw) {
        std::vector<std::string> available = strategy_cpp::StrategyRegistry::get_all_strategy_class_names();
    ...
    auto ptr = std::unique_ptr<strategy_cpp::OptionStrategyTemplate>(static_cast<strategy_cpp::OptionStrategyTemplate*>(raw));
    if (api_.get_or_create_holding) api_.get_or_create_holding(strategy_name);
    if (api_.get_holding) ptr->set_holding(api_.get_holding(strategy_name));
...
utilities::OrderData* OptionStrategyEngine::get_order(const std::string& orderid) {
    auto it = orders_.find(orderid);
    return (it != orders_.end()) ? &it->second : nullptr;
}

utilities::TradeData* OptionStrategyEngine::get_trade(const std::string& tradeid) {
    auto it = trades_.find(tradeid);
    return (it != trades_.end()) ? &it->second : nullptr;
}

std::string OptionStrategyEngine::get_strategy_name_for_order(const std::string& orderid) const {
    auto it = orderid_strategy_name_.find(orderid);
    return (it != orderid_strategy_name_.end()) ? it->second : std::string{};
}

std::vector<utilities::OrderData> OptionStrategyEngine::get_all_orders() const {
    std::vector<utilities::OrderData> out;
    for (const auto& [_, o] : orders_) out.push_back(o);
    return out;
}

std::vector<utilities::TradeData> OptionStrategyEngine::get_all_trades() const {
    std::vector<utilities::TradeData> out;
    for (const auto& [_, t] : trades_) out.push_back(t);
    return out;
}

std::vector<utilities::OrderData> OptionStrategyEngine::get_all_active_orders() const {
    std::vector<utilities::OrderData> out;
...
const std::unordered_map<std::string, std::set<std::string>>& OptionStrategyEngine::get_strategy_active_orders() {
    hedge_active_orders_cache_ = strategy_active_orders_;
    return hedge_active_orders_cache_;
}

std::vector<std::string> OptionStrategyEngine::get_strategy_names() const {
    std::vector<std::string> out;
    for (const auto& kv : strategies_) out.push_back(kv.first);
    return out;
}
...
engines::ComboBuilderEngine* OptionStrategyEngine::combo_builder_engine() {
    return api_.get_combo_builder_engine ? api_.get_combo_builder_engine() : nullptr;
}

engines::HedgeEngine* OptionStrategyEngine::hedge_engine() {
