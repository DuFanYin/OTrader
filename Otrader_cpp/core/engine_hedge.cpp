 ...
 * Same logic: timer, run_strategy_hedging, compute_hedge_plan, execute_hedge_orders, check/cancel orders.
 */
...
void HedgeEngine::register_strategy(const std::string& strategy_name,
                                    int timer_trigger,
                                    int delta_target,
                                    int delta_range) {
    HedgeConfig config;
...
void HedgeEngine::process_hedging(const std::string& strategy_name, const HedgeParams& params,
    ...
    if (!out_orders && !out_cancels && !out_logs) return;
    auto it = registered_strategies_.find(strategy_name);
    if (it == registered_strategies_.end()) return;
    run_strategy_hedging_with_params(strategy_name, it->second, params, out_orders, out_cancels, out_logs);
}

void HedgeEngine::run_strategy_hedging_with_params(const std::string& strategy_name, HedgeConfig& config, const HedgeParams& params,
                                                   std::vector<utilities::OrderRequest>* out_orders,
                                                   std::vector<utilities::CancelRequest>* out_cancels,
                                                   std::vector<utilities::LogData>* out_logs) {
    ...
    execute_hedge_orders(strategy_name, symbol, direction, available, order_volume, params, out_orders, out_logs);
}

std::optional<std::tuple<std::string, utilities::Direction, double, double>> HedgeEngine::compute_hedge_plan(
    const std::string& strategy_name, HedgeConfig& config, const HedgeParams& params) {
    (void)config;
    if (!params.holding || !params.portfolio || !params.portfolio->underlying) return std::nullopt;
    ...
    if (total_delta >= delta_min && total_delta <= delta_max) return std::nullopt;

    double delta_to_hedge = config.delta_target - total_delta;
    utilities::UnderlyingData& underlying = *params.portfolio->underlying;
    double hedge_volume = delta_to_hedge / (underlying.theo_delta != 0 ? underlying.theo_delta : 1.0);
    std::string symbol = underlying.symbol;

    if (!params.get_contract) return std::nullopt;
    const utilities::ContractData* contract = params.get_contract(symbol);
    if (!contract || std::abs(hedge_volume) < 1) return std::nullopt;
...
void HedgeEngine::execute_hedge_orders(const std::string& strategy_name, const std::string& symbol,
    utilities::Direction direction, double available, double order_volume, const HedgeParams& params,
    std::vector<utilities::OrderRequest>* out_orders, std::vector<utilities::LogData>* out_logs) {
    if (!out_orders && !out_logs) return;
    double remaining = order_volume;
    if (available > 0) {
        double close_vol = std::min(available, order_volume);
        submit_hedge_order(strategy_name, symbol, direction, close_vol, params, out_orders, out_logs);
        remaining -= close_vol;
    }
    if (remaining > 0)
        submit_hedge_order(strategy_name, symbol, direction, remaining, params, out_orders, out_logs);
}

void HedgeEngine::submit_hedge_order(const std::string& strategy_name, const std::string& symbol,
    utilities::Direction direction, double volume, const HedgeParams& params,
    std::vector<utilities::OrderRequest>* out_orders, std::vector<utilities::LogData>* out_logs) {
    if (!params.get_contract) return;
    const utilities::ContractData* contract = params.get_contract(symbol);
    if (!contract) return;
    ...
    if (out_orders) out_orders->push_back(req);

    if (out_logs) {
        utilities::LogData log;
        log.msg = std::string("Hedge sending order: dir=") + (direction == utilities::Direction::LONG ? "LONG" : "SHORT") + ", vol=" + std::to_string(volume) + ", symbol=" + symbol;
        log.level = 0;
...
bool HedgeEngine::check_strategy_orders_finished(const std::string& strategy_name, const HedgeParams& params) {
    if (!params.get_strategy_active_orders || !params.get_order) return true;
    const auto& active_map = params.get_strategy_active_orders();
    auto it = active_map.find(strategy_name);
    if (it == active_map.end()) return true;
    for (const auto& orderid : it->second) {
        utilities::OrderData* order = params.get_order(orderid);
        if (order && order->reference.find(APP_NAME) != std::string::npos)
            return false;
    }
    return true;
}

void HedgeEngine::cancel_strategy_orders(const std::string& strategy_name, const HedgeParams& params,
                                         std::vector<utilities::CancelRequest>* out_cancels) {
    if (!out_cancels || !params.get_strategy_active_orders || !params.get_order) return;
    const auto& active_map = params.get_strategy_active_orders();
    auto it = active_map.find(strategy_name);
    if (it == active_map.end()) return;
    for (const auto& orderid : it->second) {
        utilities::OrderData* order = params.get_order(orderid);
        if (order && order->reference.find(APP_NAME) != std::string::npos)
